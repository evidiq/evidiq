...
dim "  OpenClaw agent  ·  brain: 0G Compute (qwen3.7-plus)  ·  pay: x402 / X Layer"
vio "══════════════════════════════════════════════════════════════"
echo
sleep 2
...
sleep 0.6
cya "🤖 agent:  A cold-DM \"airdrop bot\" wants 250 USDC up front for \"10x returns\"."
sleep 0.8
cya "🤖 agent:  I'm not touching my wallet until EVIDIQ verifies it."
echo
sleep 1
node "$V" airdrop-bot http://sketchy-agent.invalid --context "cold DM: send 250 USDC, get 2500 back"
echo
sleep 1.5
...
sleep 0.6
cya "🤖 agent:  Different counterparty, same 250 USDC job. Verify it too."
echo
sleep 1
node "$V" 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984 https://evidiq.dev/skill.md --address 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984 --erc8004 1024 --domain evidiq.dev --capabilities "trust.verify,attestation" --context "data-enrichment job, 250 USDC"
echo
sleep 1.5
